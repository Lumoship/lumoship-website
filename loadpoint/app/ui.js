...
(function () {
  ...
  var CLASSES = [['dnv', 'DNV RU-SHIP Pt 3 (July 2026)'], ['bv', 'BV NR467 Pt B (July 2026)'], ['lrssc', 'LR SSC Pt 5-6 (July 2026)'], ['lr', 'LR Rules for Ships - not yet']];
  ...
  var defPressCol = function () { return { name: '', X: '#0', Y: 0, Z: 0, B_x: '', member: 'side', comp1: defComp('none'), comp2: defComp('sea'), scenario: 'all', showCases: false, ssc: defSSC() }; };
  ...
  var defScantCol = function () {
    return { name: '', X: '#15', Y: 0, Z: 9300, a: 2400, b: 740, comp1: defComp('none'), comp2: defComp('none'), scenario: 'all',
             ...
             ssc: defSSC(), sscMember: 'secondary', sscModel: 'auto', curv_h: 0, matSSC: 'MS (A/B/D/E)', matSSCs: 'MS (A/B/D/E)' };
  };
  ...
  function paintPressure() {
    ...
    S.press.forEach(function (col, i) {
      ...
      var res = '', err = isSSC() ? '' : Pos.conflicts(col.member, col.comp1.kind, col.comp2.kind).join(' · '), hero = [], status = null;
      try {
  ...
  function paintShip() {
    ...
    if (isSSC()) {
      var cp = '<div class="lp-cls">' + CLASSES.map(function (c) {
        var ready = c[0] === 'dnv' || c[0] === 'bv' || c[0] === 'lrssc';
  ...
  function paint() {
    ...
    $('#wizard').innerHTML = w;
    var sub = document.querySelector('.ea-title-group p'); if (sub) sub.textContent = (isBV() ? 'BV NR467 Pt B (July 2026)' : isSSC() ? 'LR Special Service Craft Pt 5-6 (July 2026)' : 'DNV RU-SHIP Pt 3 (July 2026)') + ' — design pressures, accelerations and plate / stiffener scantlings at a point';
