                ...
                <div style="position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:10000; display:flex; align-items:center; justify-content:center;" onclick="if(event.target===this)this.remove()">
                    <div style="background:var(--bg-elev); border-radius:var(--r-ovl); width:90%; max-width:900px; max-height:85vh; overflow:hidden;">
                        ...
                        <div style="display:flex; border-bottom:1px solid var(--border); background:var(--bg-main);">
                            <button class="lib-tab active" onclick="showLibraryTab('hp')" data-tab="hp" style="flex:1; padding:12px; background:transparent; border:none; color:var(--primary); cursor:pointer; border-bottom:2px solid var(--primary); font-weight:bold;">HP Bulb</button>
                                ...
                                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(130px, 1fr)); gap:8px;">
                                    ${HP_CATALOG.map(hp => `
                                        <div class="section-card" onclick="addSectionFromCatalog('HP', '${hp.name}')" style="padding:8px; background:var(--bg-main); border-radius:var(--r-ovl); cursor:pointer; text-align:center; border:2px solid ${SECTIONS[hp.name] ? 'var(--success)' : '#334155'}; transition:all 0.2s;">
                                            <div style="font-weight:bold; color:var(--primary); font-size:var(--fs-md);">${hp.name}</div>
                                ...
                                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(130px, 1fr)); gap:8px;">
                                    ${FB_CATALOG.map(fb => {
                                        ...
                                        <div class="section-card" onclick="addSectionFromCatalog('FB', '${fb.name}')" style="padding:8px; background:var(--bg-main); border-radius:var(--r-ovl); cursor:pointer; text-align:center; border:2px solid ${SECTIONS[fb.name] ? 'var(--success)' : '#334155'}; transition:all 0.2s;">
                                            <div style="font-weight:bold; color:var(--success); font-size:var(--fs-md);">${fb.name}</div>
                                ...
                                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(130px, 1fr)); gap:8px;">
                                    ${L_CATALOG.map(l => `
                                        <div class="section-card" onclick="addSectionFromCatalog('L', '${l.name}')" style="padding:8px; background:var(--bg-main); border-radius:var(--r-ovl); cursor:pointer; text-align:center; border:2px solid ${SECTIONS[l.name] ? 'var(--success)' : '#334155'}; transition:all 0.2s;">
                                            <div style="font-weight:bold; color:var(--warning); font-size:var(--fs-md);">${l.name}</div>
                                ...
                                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:8px;">
                                    ${T_CATALOG.map(t => `
                                        <div class="section-card" onclick="addSectionFromCatalog('T', '${t.name}')" style="padding:8px; background:var(--bg-main); border-radius:var(--r-ovl); cursor:pointer; text-align:center; border:2px solid ${SECTIONS[t.name] ? 'var(--success)' : '#334155'}; transition:all 0.2s;">
                                            <div style="font-weight:bold; color:var(--primary); font-size:var(--fs-md);">${t.name}</div>
                                ...
                                <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                                    ...
                                    <div style="background:var(--bg-main); padding:16px; border-radius:var(--r-ovl);">
                                        ...
                                        <div style="display:grid; gap:12px;">
                                            ...
                                            <button onclick="addCustomSection()" style="width:100%; padding:12px; background:var(--primary); color:white; border:none; border-radius:var(--r-ctl); cursor:pointer; font-weight:bold;">
                                                Add Custom Section
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div style="padding:20px;">
                            ...
                            <div style="margin-bottom:16px;">
                                ...
                                <div style="display:flex; gap:8px;">
                                    <label style="flex:1; display:flex; align-items:center; gap:8px; padding:8px; background:var(--bg-main); border-radius:var(--r-ctl); cursor:pointer; border:2px solid var(--border);" id="labelRuleBased">
                                        ...
                                        <div>
                                            <div style="color:var(--primary); font-size:var(--fs-sm); font-weight:bold;">BV NR467 Rule</div>
                                    ...
                                    <label style="flex:1; display:flex; align-items:center; gap:8px; padding:8px; background:var(--bg-main); border-radius:var(--r-ctl); cursor:pointer; border:2px solid var(--border);" id="labelCustom">
                                        ...
                                        <div>
                                            <div style="color:var(--warning); font-size:var(--fs-sm); font-weight:bold;">Custom</div>
                            ...
                            <div id="beffRuleInputs">
                                ...
                                <div style="background:linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%); padding:12px; border-radius:var(--r-ovl); margin-bottom:12px;">
                                    <div style="display:flex; justify-content:space-between; align-items:center;">
                                        ...
                                        <span style="color:var(--success); font-size:var(--fs-lg); font-weight:bold;" id="beffCalculated">500 mm</span>
                                    </div>
                            ...
                            <div style="background:var(--bg-main); padding:12px; border-radius:var(--r-ovl); margin-bottom:16px; border:1px solid var(--border);">
                                ...
                                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; font-size:var(--fs-md);">
                                    <div><span style="color:var(--text-3);">A<sub>comp</sub>:</span> <span style="color:var(--success); font-weight:bold;" id="compA">--</span></div>
                                    <div><span style="color:var(--text-3);">h<sub>total</sub>:</span> <span style="color:var(--text);" id="compH">--</span></div>
                                    <div><span style="color:var(--text-3);">I<sub>y,comp</sub>:</span> <span style="color:var(--success); font-weight:bold;" id="compIy">--</span></div>
                                    <div><span style="color:var(--text-3);">ȳ (from plate):</span> <span style="color:var(--text);" id="compYbar">--</span></div>
                                    <div><span style="color:var(--text-3);">W<sub>y,deck</sub>:</span> <span style="color:var(--warning); font-weight:bold;" id="compWyDeck">--</span></div>
                                    <div><span style="color:var(--text-3);">W<sub>y,flange</sub>:</span> <span style="color:var(--warning); font-weight:bold;" id="compWyFlange">--</span></div>
                                </div>
                            </div>
                            ...
                            <div style="display:flex; gap:12px;">
                                ...
                                <button onclick="addSectionWithPlate('${profileType}', '${profileName}')" style="flex:1; padding:12px; background:var(--success); color:white; border:none; border-radius:var(--r-ctl); cursor:pointer; font-weight:bold;">
                                    ✓ Add to Model
                                </button>
                            </div>
                        </div>
            ...
            if (bcTable) {
                ...
                if (constraints.length === 0) {
                    ...
                    bcTable.innerHTML = constraints.map(([nodeId, bc]) => {
                        ...
                        const check = (val) => val ? '<span style="background:var(--success); color:white; padding:1px 4px; border-radius:var(--r-ctl); font-weight:bold;"><span class="icon"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg></span></span>' : '<span style="color:var(--text-3);">-</span>';
                        return `<tr>
                            <td style="color:var(--primary); font-weight:bold;">${nodeId}</td>
            ...
            if (loadTable) {
                if (model.loads.length === 0) {
                    ...
                    loadTable.innerHTML = model.loads.map((load, idx) => {
                        ...
                        return `<tr>
                            <td style="color:var(--primary); font-weight:bold;">${load.nodeId}</td>
                            ...
                            <td onclick="editPointLoad(${idx}, 'Fz')" style="color:${clr(load.Fz)}; font-weight:bold; ${editStyle}" title="Click to edit">${fmt(load.Fz)}</td>
            ...
            if (lineLoadTable) {
                ...
                if (lineLoads.length === 0) {
                    ...
                    lineLoadTable.innerHTML = lineLoads.map(ll => {
                        ...
                        return `<tr>
                            <td style="color:var(--primary); font-weight:bold;">E${ll.elemId}</td>
                            <td style="color:var(--text-3); font-size:var(--fs-xs);">${ll.n1}→${ll.n2}</td>
                            <td onclick="editLineLoad(${ll.elemId}, ${ll.idx})" style="color:var(--success); font-weight:bold; cursor:pointer; text-decoration:underline; text-decoration-style:dotted;" title="Click to edit">${ll.value.toFixed(1)}</td>
