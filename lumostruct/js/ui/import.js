        ...
        function updateBCLoadsTable() {
            ...
            // Kiris tekil yukleri tablosu (Loads sekmesi) - eskiden yalnizca kiris
            // panelinde gorunuyordu; toplu bakis ve durum atama yoktu.
            const noktaTablo = document.getElementById('kirisNoktaYukTablosu');
            if (noktaTablo) {
                const satirlar = [];
                Object.entries(model.elements).forEach(([elemKey, elem]) => {
                    (elem.pointLoads || []).forEach((p, idx) => {
                ...
                noktaTablo.innerHTML = satirlar.length ? satirlar.join('') : '<tr><td colspan="6" style="color:var(--text-3); text-align:center;">No beam point loads (add from the beam panel)</td></tr>';
            }
