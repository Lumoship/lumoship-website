...
(function () {
  ...
  function buildModel() {
    ...
    const ship = { L, B, D: Dd, TSC: T, TBAL: TBAL(), CB, rollType: bulk ? 'bulkFull' : 'general', bilgeKeel: (($('bvBilgeKeel') || {}).value || 'Yes') !== 'No', v: num('serviceSpeed', undefined),
      ...
      holdsMayBeEmpty: !!($('notHoldsEmpty') && $('notHoldsEmpty').checked), sideFrameCSR: !!($('dnv_sideFrameCSR') && $('dnv_sideFrameCSR').checked) };
    ...
    Object.keys(s.groups || {}).forEach(gid => {
      ...
      stPos.forEach((sp, i) => {
        ...
        stiffs.push({ id: gid + '/' + sp.g.id + '#' + (i + 1), panel: plate.pos, plateId: plate.id, y: at.y, z: at.z, ref: { y: at.y + nx * pr.hw, z: at.z + nz * pr.hw }, type: pr.type, hw: pr.hw, tw: pr.tw, bf: pr.bf, tf: pr.tf, tcw, tcf, ReH, vertical: Math.abs(dz) > Math.abs(dy), dir: sp.g.dir === 'trans' ? 'trans' : 'long',
